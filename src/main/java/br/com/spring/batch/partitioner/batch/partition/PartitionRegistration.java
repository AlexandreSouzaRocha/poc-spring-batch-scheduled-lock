...
public class PartitionRegistration {
    ...
    public List<ReceivedFileDocument> registerUploaded(ReceivedFileDocument original) {
        ...
        List<ReceivedFileDocument> partitions = PartitionPlan.split(original.lineCount(), original.partitionCount(), layout)
                .ranges().stream()
    ...
    private ReceivedFileDocument toDocument(ReceivedFileDocument original, PartitionRange range,
        ...
        return original.uploadedPartition(range, paths.partitionFileName(original, range.index()), path,
                range.fileSizeBytes(layout), now);
    }

    private void requireUploaded(String path, PartitionRange range, Optional<BlobFile> blob) {
        long expected = range.fileSizeBytes(layout);
        boolean complete = blob.map(file -> file.sizeBytes() == expected).orElse(false);
        if (!complete) {
            throw new IllegalStateException("partição " + path + " ausente ou incompleta no blob; esperado "
                    + expected + " bytes, encontrado " + blob.map(BlobFile::sizeBytes).orElse(0L));
        }
    }
}
